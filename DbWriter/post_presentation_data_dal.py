...
from DbWriter.social_media_dto import PostPresentationData
...
class PostPresentationDataDAL():
    ...
    async def create_post_presentation_data(self, source: str, content: str) -> PostPresentationData:
        new_ppd = PostPresentationData(
            source, content, added_on=datetime.utcnow(), last_updated_on=datetime.utcnow())
    ...
    async def get_post_presentation_data_by_source(self, source: str) -> List[PostPresentationData]:
        q = await self.db_session.execute(
            select(PostPresentationData).where(PostPresentationData.source == source))
        return q.scalars().all()
